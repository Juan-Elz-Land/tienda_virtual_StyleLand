   ...
   Mostrar_Menu_Opciones()
    ...
    function Mostrar_Menu_Opciones(){
        let op = opciones.map(op => `<li class="${op.Class}"> <a id="${op.id}" onmouseover="Opcion_Menu('${op.id}')" href="${op.Url}">${op.Page}</a><i class="${op.Font_Aw}"></i></li>`)
        ...
        let s_op = sub_opciones.map(s_op => `<li> <i class="${s_op.Class}"></i><span onmouseover="Mostrar_Producto_Categ('${s_op.Name}')"> ${s_op.Name}</span></li>`)
        sub_menu.innerHTML += `
